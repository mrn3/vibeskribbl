...
exports.AnalyticsProcessor = exports.AnalyticsCollector = void 0;
const uuid_1 = require("uuid");
// In-memory storage for analytics data
// In a production environment, this would be replaced with a database
let gameAnalytics = new Map();
let playerGuesses = [];
class AnalyticsCollector {
    // Start tracking a new game
    static startGame(roomId, players) {
    ...
    static startRound(gameId, roundNumber, word, drawer, totalPlayers) {
        const game = gameAnalytics.get(gameId);
        if (game) {
            const roundAnalytic = {
                gameId,
    ...
    static recordGuess(gameId, roundNumber, player, guessTime, pointsEarned) {
        const game = gameAnalytics.get(gameId);
        if (game) {
    ...
    // End round tracking
    static endRound(gameId, roundNumber, roundDuration) {
        const game = gameAnalytics.get(gameId);
        if (game) {
            const round = game.rounds.find(r => r.roundNumber === roundNumber);
            if (round) {
    ...
    static getGameAnalytics(gameId) {
...
class AnalyticsProcessor {
    ...
    static calculateGameStats(filters) {
        let games = Array.from(gameAnalytics.values());
        // Apply filters
        if (filters) {
            if (filters.dateRange) {
                games = games.filter(game => game.startTime >= filters.dateRange.start.getTime() &&
    ...
    static calculatePlayerStats(filters) {
        let games = Array.from(gameAnalytics.values());
        ...
        if (filters) {
            ...
            if (filters.playerIds) {
                games = games.filter(game => game.rounds.some(round => round.guesses.some(guess => filters.playerIds.includes(guess.playerId))));
            }
        }
    ...
    static generateResponseTimeChartData(filters) {
        let guesses = [...playerGuesses];
        if (filters) {
            if (filters.dateRange) {
                guesses = guesses.filter(guess => guess.timestamp >= filters.dateRange.start.getTime() &&
        ...
        guesses.forEach(guess => {
            if (guess.guessTime <= 5)
                counts[0]++;
            else if (guess.guessTime <= 10)
                counts[1]++;
            else if (guess.guessTime <= 15)
                counts[2]++;
            else if (guess.guessTime <= 20)
                counts[3]++;
            else
                counts[4]++;
        });
