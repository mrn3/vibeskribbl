...
export default function LiveStatsCard({ refreshInterval = 30000 }: LiveStatsCardProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const mockStats: LiveStats = {
          ...
          averageResponseTime: data.playerStats?.length > 0 
            ? data.playerStats.reduce((sum: number, p: any) => sum + p.averageGuessTime, 0) / data.playerStats.length
            : 0,
          lastUpdated: Date.now()
        };
      ...
      <div className="p-6">
        ...
        <div className="mt-4 flex space-x-2">
          ...
          <button
            ...
            className="flex-1 px-3 py-2 text-xs font-medium text-green-600 bg-green-50 rounded-md hover:bg-green-100"
          >
            Join Game
          </button>
        </div>
