...
import { PlayerPerformanceStats } from '@/types/analytics';

interface PlayerPerformanceTableProps {
  players: PlayerPerformanceStats[];
  sortBy?: keyof PlayerPerformanceStats;
  sortOrder?: 'asc' | 'desc';
  onSort?: (field: keyof PlayerPerformanceStats) => void;
}

export default function PlayerPerformanceTable({ 
  players, 
  sortBy = 'totalPointsEarned', 
  sortOrder = 'desc',
  onSort 
  ...
  const getSortIcon = (field: keyof PlayerPerformanceStats) => {
    if (sortBy !== field) {
      return (
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th 
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                onClick={() => handleSort('totalCorrectGuesses')}
              >
                <div className="flex items-center space-x-1">
              ...
              <th 
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                onClick={() => handleSort('averageGuessTime')}
              >
                <div className="flex items-center space-x-1">
              ...
              <th 
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                onClick={() => handleSort('fastestGuessTime')}
              >
                <div className="flex items-center space-x-1">
